...
static const char *bufferwriter_get_name(void *ctx) { return WRITER_NAME; }
...
static void buffer_w8(AVTextWriterContext *wctx, int b) {
  BufferWriterContext *ctx = (BufferWriterContext *)wctx->priv;
...
const AVTextWriter avtextwriter_buffer = {.name = WRITER_NAME,
                                          .priv_size =
                                              sizeof(BufferWriterContext),
...
int avtextwriter_create_buffer(AVTextWriterContext **pwctx, AVBPrint *buffer) {
  ...
  ctx = (BufferWriterContext *)(*pwctx)->priv;
  ctx->buffer = buffer;
