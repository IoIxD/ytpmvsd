...
static av_cold void iowriter_uninit(AVTextWriterContext *wctx) {
  IOWriterContext *ctx = (IOWriterContext *)wctx->priv;
...
static void io_put_str(AVTextWriterContext *wctx, const char *str) {
  IOWriterContext *ctx = (IOWriterContext *)wctx->priv;
  avio_write(ctx->avio_context, (const unsigned char *)str, strlen(str));
}
...
const AVTextWriter avtextwriter_avio = {.name = WRITER_NAME,
...
int avtextwriter_create_file(AVTextWriterContext **pwctx,
  ...
  ctx = (IOWriterContext *)(*pwctx)->priv;

  if ((ret = avio_open(&ctx->avio_context, output_filename, AVIO_FLAG_WRITE)) <
      0) {
    av_log(ctx, AV_LOG_ERROR, "Failed to open output '%s' with error: %s\n",
...
int avtextwriter_create_avio(AVTextWriterContext **pwctx, AVIOContext *avio_ctx,
                             int close_on_uninit) {
  IOWriterContext *ctx;
  int ret;
