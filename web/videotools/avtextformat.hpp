...
#define AV_TEXTFORMAT_SECTION_FLAG_NUMBERING_BY_TYPE                           \
  16 ///< the items in this array section should be numbered individually by
     ///< type

  int flags;
  const int children_ids[SECTION_MAX_NB_CHILDREN +
  ...
  const char *(*get_type)(
      const void *data); ///< function returning a type if defined, must be
                         ///< defined when SECTION_FLAG_HAS_TYPE is defined
...
struct AVTextFormatContext {
  const AVClass *cls; ///< class of the formatter
  const AVTextFormatter
      *formatter; ///< the AVTextFormatter of which this is an instance
