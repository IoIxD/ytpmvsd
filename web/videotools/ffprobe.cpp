...
#define print_str(k, v) avtext_print_string(tfc, k, (const char *)v, 0)
...

  {
    ...
    for (int c = 0; c < 3; c++) {
      ...
      for (int i = 0; i < curve->num_pivots - 1; i++) {
        ...
        avtext_print_section_header(tfc, piece_buf.str,
                                    SECTION_ID_FRAME_SIDE_DATA_PIECE);
...
static void print_film_grain_params(AVTextFormatContext *tfc,
  ...
  case AV_FILM_GRAIN_PARAMS_AV1: {
    ...
    for (int uv = 0; uv < 2; uv++) {
      ...
      print_list_fmt("uv_points_value", "%" PRIu8, aom->num_uv_points[uv], 1,
                     aom->uv_points[uv][idx][0]);
      print_list_fmt("uv_points_scaling", "%" PRIu8, aom->num_uv_points[uv], 1,
  ...
  case AV_FILM_GRAIN_PARAMS_H274: {
    ...
    for (int c = 0; c < 3; c++) {
      ...
      avtext_print_section_header(tfc, NULL,
                                  SECTION_ID_FRAME_SIDE_DATA_COMPONENT);
      print_int(c ? "bit_depth_chroma" : "bit_depth_luma",
      ...
      for (int i = 0; i < h274->num_intensity_intervals[c]; i++) {
        ...
        avtext_print_section_header(tfc, NULL,
                                    SECTION_ID_FRAME_SIDE_DATA_PIECE);
        print_int("intensity_interval_lower_bound",
                  h274->intensity_interval_lower_bound[c][i]);
        print_int("intensity_interval_upper_bound",
                  h274->intensity_interval_upper_bound[c][i]);
        print_list_fmt("comp_model_value", "%" PRId16,
                       h274->num_model_values[c], 1,
                       h274->comp_model_value[c][i][idx]);
...
static void show_subtitle(AVTextFormatContext *tfc, AVSubtitle *sub,
  ...
  print_time("pts_time", sub->pts, &time_q);
...
static void print_frame_side_data(AVTextFormatContext *tfc,
  ...
  for (int i = 0; i < frame->nb_side_data; i++) {
    ...
    if (sd->type == AV_FRAME_DATA_DISPLAYMATRIX && sd->size >= 9 * 4) {
      ...
      if (isnan(rotation))
        rotation = 0;
      avtext_print_integers(tfc, "displaymatrix", sd->data, 9, " %11d", 3, 4,
                            1);
      print_int("rotation", rotation);
...
static void show_frame(AVTextFormatContext *tfc, AVFrame *frame,
  ...
  case AVMEDIA_TYPE_VIDEO:
    ...
    print_pixel_format(tfc, (enum AVPixelFormat)frame->format);
...
static int read_interval_packets(AVTextFormatContext *tfc, InputFile *ifile,
  ...
  for (i = 0; i < ifile->nb_streams; i++) {
    pkt->stream_index = i;
    auto wh = (int){1};
    while (process_frame(tfc, ifile, frame, pkt, &wh) > 0)
      ;
...
static int show_stream(AVTextFormatContext *tfc, AVFormatContext *fmt_ctx,
  ...
  case AVMEDIA_TYPE_VIDEO:
    ...
    // }
    print_pixel_format(tfc, (enum AVPixelFormat)par->format);
...
static int show_program(AVTextFormatContext *tfc, InputFile *ifile,
  ...
  for (i = 0; i < program->nb_stream_indexes; i++) {
    if (selected_streams[program->stream_index[i]]) {
      ret = show_stream(tfc, fmt_ctx, program->stream_index[i],
...
static void print_tile_grid_params(AVTextFormatContext *tfc,
  ...
  for (int i = 0; i < tile_grid->nb_tiles; i++) {
    avtext_print_section_header(tfc, "tile_offset",
                                SECTION_ID_STREAM_GROUP_SUBCOMPONENT);
...
static void print_iamf_param_definition(AVTextFormatContext *tfc,
  ...
  parameter_section_id = (SectionID)sections[subsection_id].children_ids[0];
  ...
  for (int i = 0; i < param->nb_subblocks; i++) {
    ...
    case AV_IAMF_PARAMETER_DEFINITION_MIX_GAIN: {
      ...
      avtext_print_section_header(tfc, "IAMF Mix Gain Parameters",
    ...
    case AV_IAMF_PARAMETER_DEFINITION_DEMIXING: {
      ...
      avtext_print_section_header(tfc, "IAMF Demixing Info",
                                  parameter_section_id);
...
print_iamf_audio_element_params(AVTextFormatContext *tfc,
  ...
  for (int i = 0; i < audio_element->nb_layers; i++) {
    ...
    char val_str[128];
    avtext_print_section_header(tfc, "IAMF Audio Layer",
                                SECTION_ID_STREAM_GROUP_SUBCOMPONENT);
  ...
  if (audio_element->demixing_info)
    print_iamf_param_definition(tfc, "demixing_info",
                                audio_element->demixing_info,
                                SECTION_ID_STREAM_GROUP_SUBCOMPONENT);
  if (audio_element->recon_gain_info)
    print_iamf_param_definition(tfc, "recon_gain_info",
...
static void print_iamf_submix_params(AVTextFormatContext *tfc,
  ...
  for (int i = 0; i < submix->nb_elements; i++) {
    ...
    avtext_print_section_header(tfc, "IAMF Submix Element",
                                SECTION_ID_STREAM_GROUP_PIECE);
    ...
    if (element->annotations) {
      ...
      avtext_print_section_header(tfc, "IAMF Annotations",
                                  SECTION_ID_STREAM_GROUP_SUBPIECE);
    ...
    if (element->element_mix_config)
      print_iamf_param_definition(tfc, "element_mix_config",
...
static void print_stream_group_params(AVTextFormatContext *tfc,
  ...
  if (stg->type == AV_STREAM_GROUP_PARAMS_TILE_GRID)
    ...
    print_iamf_mix_presentation_params(tfc, stg,
...
static int show_stream_group(AVTextFormatContext *tfc, InputFile *ifile,
  ...
  for (i = 0; i < stg->nb_streams; i++) {
    if (selected_streams[stg->streams[i]->index]) {
      ret =
          show_stream(tfc, fmt_ctx, stg->streams[i]->index,
...
static int show_format(AVTextFormatContext *tfc, InputFile *ifile) {
  ...
  print_time("start_time", fmt_ctx->start_time, &time_q);
  print_time("duration", fmt_ctx->duration, &time_q);
  if (size >= 0)
...
static inline void mark_section_show_entries(SectionID section_id,
  ...
  if (show_all_entries) {
    for (const int *id = section->children_ids; *id != -1; id++)
      mark_section_show_entries((SectionID)*id, show_all_entries, entries);
  } else {
...
static int match_section(const char *section_name, int show_all_entries,
  ...
  for (i = 0; i < FF_ARRAY_ELEMS(sections); i++) {
    ...
    if (!strcmp(section_name, section->name) ||
      ...
      ret++;
      mark_section_show_entries((SectionID)section->id, show_all_entries,
                                entries);
    }
  }
  return ret;
