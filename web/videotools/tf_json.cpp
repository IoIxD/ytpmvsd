...
static void json_print_section_header(AVTextFormatContext *wctx,
  ...
  const struct AVTextFormatSection *parent_section =
  ...
  if (section->flags & AV_TEXTFORMAT_SECTION_FLAG_IS_WRAPPER) {
    ...
    if (section->flags & AV_TEXTFORMAT_SECTION_FLAG_IS_ARRAY) {
      writer_printf(wctx, "\"%s\": [\n", buf.str);
               ...
               !(parent_section->flags & AV_TEXTFORMAT_SECTION_FLAG_IS_ARRAY)) {
...
static void json_print_str(AVTextFormatContext *wctx, const char *key,
                           const char *value) {
  JSONContext *json = (JSONContext *)wctx->priv;
  const struct AVTextFormatSection *parent_section =
      wctx->level ? wctx->section[wctx->level - 1] : NULL;
  ...
  if (wctx->nb_item[wctx->level] ||
      (parent_section &&
       parent_section->flags & AV_TEXTFORMAT_SECTION_FLAG_NUMBERING_BY_TYPE))
...
static void json_print_int(AVTextFormatContext *wctx, const char *key,
                           int64_t value) {
  JSONContext *json = (JSONContext *)wctx->priv;
  const struct AVTextFormatSection *parent_section =
      wctx->level ? wctx->section[wctx->level - 1] : NULL;
  ...
  av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
  writer_printf(wctx, "\"%s\": %" PRId64, json_escape_str(&buf, key, wctx),
                value);
  av_bprint_finalize(&buf, NULL);
}
