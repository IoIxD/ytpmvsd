...
static av_cold int flat_init(AVTextFormatContext *wctx) {
  FlatContext *flat = (FlatContext *)wctx->priv;
...
static void flat_print_int(AVTextFormatContext *wctx, const char *key,
                           int64_t value) {
  writer_printf(wctx, "%s%s=%" PRId64 "\n", wctx->section_pbuf[wctx->level].str,
                key, value);
}

static void flat_print_str(AVTextFormatContext *wctx, const char *key,
                           const char *value) {
  FlatContext *flat = (FlatContext *)wctx->priv;
  AVBPrint buf;
