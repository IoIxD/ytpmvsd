...
static const char *textcontext_get_formatter_name(void *p) {
  AVTextFormatContext *tctx = (AVTextFormatContext *)p;
...
static const AVOption textcontext_options[] = {
    {"string_validation",
     ...
     AV_OPT_TYPE_INT,
     {.i64 = AV_TEXTFORMAT_STRING_VALIDATION_REPLACE},
     0,
     AV_TEXTFORMAT_STRING_VALIDATION_NB - 1,
     0,
     "sv"},
    {"sv",
     "set string validation mode",
    ...
    {"ignore",
     ...
     0,
     AV_OPT_TYPE_CONST,
     {.i64 = AV_TEXTFORMAT_STRING_VALIDATION_IGNORE},
     0,
     0,
     0,
     "sv"},
    ...
    {"fail",
     ...
     0,
     AV_OPT_TYPE_CONST,
     {.i64 = AV_TEXTFORMAT_STRING_VALIDATION_FAIL},
     0,
     0,
     0,
     "sv"},
...
static void *textcontext_child_next(void *obj, void *prev) {
  AVTextFormatContext *ctx = (AVTextFormatContext *)obj;
...
int avtext_context_open(AVTextFormatContext **ptctx,
                        ...
                        AVTextWriterContext *writer_context, const char *args,
                        const struct AVTextFormatSection *sections,
                        int nb_sections, int show_value_unit,
                        int use_value_prefix, int use_byte_value_binary_prefix,
                        int use_value_sexagesimal_format,
                        int show_optional_fields, char *show_data_hash) {
  AVTextFormatContext *tctx;
  int i, ret = 0;

  if (!(tctx =
            (AVTextFormatContext *)av_mallocz(sizeof(AVTextFormatContext)))) {
    ret = AVERROR(ENOMEM);
    goto fail;
  }
  ...
  if (nb_sections > SECTION_MAX_NB_SECTIONS) {
    av_log(tctx, AV_LOG_ERROR,
           "The number of section definitions (%d) is larger than the maximum "
           "allowed (%d)\n",
  ...
  if (args) {
    ...
    while ((opt = av_dict_iterate(opts, opt))) {
      if ((ret = av_opt_set(tctx, opt->key, opt->value,
  ...
  {
    const uint8_t *p = (const uint8_t *)tctx->string_validation_replacement;
...
void avtext_print_section_header(AVTextFormatContext *tctx, const void *data,
  ...
  memset(tctx->nb_item_type[tctx->level], 0,
         sizeof(tctx->nb_item_type[tctx->level]));
...
void avtext_print_section_footer(AVTextFormatContext *tctx) {
  ...
  int parent_section_id =
      tctx->level ? tctx->section[tctx->level - 1]->id : SECTION_ID_NONE;
...
static inline int validate_string(AVTextFormatContext *tctx, char **dstp,
  ...
  endp = src + strlen((const char *)src);
  for (p = src; *p;) {
    uint32_t code;
    int invalid = 0;
    const char *p0 = p;

    if (av_utf8_decode((int32_t *)&code, (const uint8_t **)&p,
                       (const uint8_t *)endp,
                       tctx->string_validation_utf8_flags) < 0) {
      AVBPrint bp;
      av_bprint_init((AVBPrint *)&bp, 0, AV_BPRINT_SIZE_AUTOMATIC);
      bprint_bytes(&bp, (const uint8_t *)p0, p - p0);
    ...
    if (!invalid ||
        tctx->string_validation == AV_TEXTFORMAT_STRING_VALIDATION_IGNORE)
      av_bprint_append_data(&dstbuf, p0, p - p0);
  }

  if (invalid_chars_nb &&
      tctx->string_validation == AV_TEXTFORMAT_STRING_VALIDATION_REPLACE) {
...
static char *value_string(AVTextFormatContext *tctx, char *buf, int buf_size,
  ...
  if (uv.unit == unit_second_str && tctx->use_value_sexagesimal_format) {
    ...
    av_strlcatf(buf, buf_size, "%s%s%s",
                *prefix_string || tctx->show_value_unit ? " " : "",
...
void avtext_print_unit_int(AVTextFormatContext *tctx, const char *key,
  ...
  uv.val.i = value;
  uv.unit = unit;
  avtext_print_string(tctx, key,
                      value_string(tctx, val_str, sizeof(val_str), uv), 0);
}
...
void avtext_print_data_hash(AVTextFormatContext *tctx, const char *name,
  ...
  p = buf + strlen(buf);
  av_hash_final_hex(tctx->hash, (uint8_t *)p, buf + sizeof(buf) - p);
...
static const char *writercontext_get_writer_name(void *p) {
  AVTextWriterContext *wctx = (AVTextWriterContext *)p;
...
int avtextwriter_context_open(AVTextWriterContext **pwctx,
  ...
  if (!(wctx =
            (AVTextWriterContext *)av_mallocz(sizeof(AVTextWriterContext)))) {
    ret = AVERROR(ENOMEM);
    goto fail;
  }
