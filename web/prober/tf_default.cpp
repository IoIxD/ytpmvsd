...
static void default_print_section_header(AVTextFormatContext *wctx,
                                         const void *data) {
  DefaultContext *def = (DefaultContext *)wctx->priv;
  char buf[32];
  ...
  if (parent_section &&
    ...
    av_bprintf(&wctx->section_pbuf[wctx->level],
               "%s%s:", wctx->section_pbuf[wctx->level - 1].str,
               upcase_string(buf, sizeof(buf),
                             (const char *)av_x_if_null(section->element_name,
                                                        section->name)));
  }
...
static void default_print_str(AVTextFormatContext *wctx, const char *key,
                              const char *value) {
  DefaultContext *def = (DefaultContext *)wctx->priv;
...
static void default_print_int(AVTextFormatContext *wctx, const char *key,
                              int64_t value) {
  DefaultContext *def = (DefaultContext *)wctx->priv;
